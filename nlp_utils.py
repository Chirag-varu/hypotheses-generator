...
def parse_natural_language_fact(sentence: str):
    ...
    if 'has property' in s:
        ...
        if y:
            facts.append(('has_property', subj, y))
    elif ' has ' in s:
    ...
    if ' can ' in s:
        ...
        if 'use sunlight' in rest:
            facts.append(('can', subj, 'use_sunlight'))
        elif y:
            facts.append(('can', subj, y))
    ...
    if 'makes sound' in s:
        ...
        if y:
            facts.append(('makes_sound', subj, y))
    ...
    if 'has symptom' in s:
        ...
        y = lemma(s[idx:].strip().split()[0]) if s[idx:].strip().split() else ''
        if y:
            facts.append(('has_symptom', subj, y))
